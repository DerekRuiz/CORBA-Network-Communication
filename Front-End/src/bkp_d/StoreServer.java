...
public class StoreServer implements StoreServerInterface {
    ...
    public String exchangeItem(String customerID, String newItemID, String oldItemID, String dateOfExchange) {
        ...
        String purchaseResult = this.purchaseItem(customerID, newItemID, dateOfExchange);
    ...
    private String[] findForeignItem(String customerID, String itemName) {
        ...
        try {
            ...
            switch (customerID.toUpperCase().substring(0, 2)) {
                case "QC":
                    request1 = new DatagramPacket(resultBytes, resultBytes.length, InetAddress.getLocalHost(), getBasePort(this.location) + 20);
                    request2 = new DatagramPacket(resultBytes, resultBytes.length, InetAddress.getLocalHost(), getBasePort(this.location) + 30);

                    break;
                case "ON":
                    request1 = new DatagramPacket(resultBytes, resultBytes.length, InetAddress.getLocalHost(), getBasePort(this.location) + 10);
                    request2 = new DatagramPacket(resultBytes, resultBytes.length, InetAddress.getLocalHost(), getBasePort(this.location) + 30);

                    break;
                case "BC":
                    request1 = new DatagramPacket(resultBytes, resultBytes.length, InetAddress.getLocalHost(), getBasePort(this.location) + 10);
                    request2 = new DatagramPacket(resultBytes, resultBytes.length, InetAddress.getLocalHost(), getBasePort(this.location) + 20);
                    break;
                default:
    ...
    private String purchaseForeignItem(String customerID, String itemID, String dateOfPurchase) {
        try (DatagramSocket bSocket = new DatagramSocket()) {
            ...
            switch (itemID.toUpperCase().substring(0, 2)) {
                case "QC":
                    request = new DatagramPacket(resultBytes, resultBytes.length, InetAddress.getLocalHost(), getBasePort(this.location) + 10);
                    break;
                case "ON":
                    request = new DatagramPacket(resultBytes, resultBytes.length, InetAddress.getLocalHost(), getBasePort(this.location) + 20);
                    break;
                case "BC":
                    request = new DatagramPacket(resultBytes, resultBytes.length, InetAddress.getLocalHost(), getBasePort(this.location) + 30);
                    break;
                default:
            ...
            System.out.println("PURCHASE FOREIGN SENT");
    ...
    private int getBasePort(String region) {
    	if (region.equalsIgnoreCase("QC"))
    		return this.port - 10;
    	else if (region.equalsIgnoreCase("ON"))
    		return this.port - 20;
    	else if (region.equalsIgnoreCase("BC"))
    		return this.port - 30;
    	else
    		return 0;
    }
    ...
    private void waitForRequest(int UDPPort) {
        ...
        try {
            ...
            switch (args[0].trim()) {
                case "PURCHASE":
                	System.out.println("PURCHASE FOREIGN RECIEVED");
